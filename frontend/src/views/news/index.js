...
export default class News extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.dispatch(
      allActionDucer(SPORTSBOOK_ANY, { activeView: "News", loadSports: true })
    );
    if (
      undefined !== this.props.sportsbook.sessionData.sid &&
      undefined === this.props.sportsbook.data.sport &&
  ...
  readNewsItem(title, id) {
    this.props.history.push(
      `${this.props.match.path}/${title.split(" ").join("-")}`,
      { id: id }
    );
  }
  render() {
        ...
        return (
          <div key={news.id} className="news-single-item">
            <div
              ...
              onClick={() =>
                this.readNewsItem(decodeHTMLEntities(news.post_title), news.id)
              }
            >
              <span>{decodeHTMLEntities(news.post_title)}</span>
            </div>
            <div className="news-content">
              <p
                dangerouslySetInnerHTML={{
                  __html: decodeHTMLEntities(news.post_excerpt),
                }}
              ></p>
            </div>
            <div className="news-date">
              <span>{new Date(news.published_time).toDateString()}</span>
            </div>
          </div>
        );
      });
    this.moreStories = this.state.newList
      .slice(this.state.newList.length - 5, this.state.newList.length)
      .map((news, ind) => {
        return (
          <div
            key={news.id}
            ...
            <div className="sportsbook-content">
              ...
              >
                <Switch>
                  ...
                  <Route
                    path={`${this.props.match.path}/:newsID`}
                    render={(props) => <NewsRead {...props} />}
                  />
                </Switch>
                <div className="news-nav col-sm-12">
                  <div className="top-news col-sm-12">
                    ...
                    <div className="item col-sm-12">
                      ...
                      <div className="title col-sm-10">
                        <span>
                          Man Utd Barca Join Chase For 17yo African Sensation
                        </span>
                      </div>
                    </div>
                    <div className="item col-sm-12">
                  ...
                  <div className="more-news col-sm-12">
                    <div className="header">
                      <div className="title">More Stories</div>
                    </div>
                    {this.moreStories}
                  </div>
                </div>
