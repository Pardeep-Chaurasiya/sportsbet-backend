...
class Header extends React.Component {
  ...
  //       web3Instance,
  //       accountsList[0],
  //       "sdfafdasdfasdfdasfasfafafafafaf"
  //     );

  //     // Check if the walletToken is stored in local storage
  //     const storedToken = localStorage.getItem("walletToken");
  //     if (!storedToken) {
  //       // Generate a new token
  //       const token = await Web3Token.sign(
  //         (msg) => web3Instance.eth.personal.sign(msg, walletId),
  //         "1d"
  //       );

  //       // Store the token along with a timestamp in local storage
  //       const tokenData = {
  //         token,
  //         timestamp: Date.now(),
  //       };
  ...
  //       // Register with the wallet and set up a timer to periodically check for token expiration
  //       $api.registerWithWallet({ walletId }, this.onLoginSuccess.bind(this));

  //       // Set up a timer to periodically check for token expiration (e.g., every hour)
  //       setInterval(() => {
